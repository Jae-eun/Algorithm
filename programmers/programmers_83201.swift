...
func solution(_ scores: [[Int]]) -> String {
    ...
    for i in scores.indices {
        ...
        if temp.max()! == selfScore || temp.min()! == selfScore {
            if temp.filter({ $0 == selfScore }).count == 1 {
                if let index = temp.firstIndex(of: selfScore) {
                    temp.remove(at: index)
                }
            }
        }
        answer += Grade(score: temp.reduce(0, +) / temp.count).rawValue
    }
    return answer
}
