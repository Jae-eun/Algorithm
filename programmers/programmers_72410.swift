//
//  programmers_72410.swift
//  algo
//
//  Created by 이재은 on 2021/01/31.
...
extension String {
    func removeSpecialChars() -> String {
        ...
        return String(self.filter { possibleChars.contains($0) })
    }

    func replaceRepeatedPeriod() -> String {
        var text = self
        while text.contains("..") {
    ...
    func removeExceededText() -> String {
        var text = self
        if self.count >= 16 {
            let removeCount = self.count - 15
            text = String(self.dropLast(removeCount))
        }
        return text
    }

    func addTextWhenShort() -> String {
        var text = self
        while text.count < 3 {
            text.append(text.last!)
        }
        return text
    }
}
