...
//programmers 60057 문자열 압축 카카오 2020
//
//문제 설명
//
...
func compress(_ n: Int, _ s: String) -> Int {
    ...
    for i in stride(from: n, to: s.count, by: n) {
        if i + n > s.count {
            compression += s[i..<s.count]
            break
        }
        ...
        if previous == s[i..<i+n] {
            ...
            compression += insertCompression(count, previous)
            count = 1
            previous = s[i..<i+n]
        }
    }
...
func insertCompression(_ count: Int, _ s: String) -> String {
    if count != 1 {
        return "\(count)" + s
    } else {
        return s
