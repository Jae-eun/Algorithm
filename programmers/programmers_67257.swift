...
// programmers 67257 [카카오 인턴 2020] 수식 최대화
...
func solution(_ expression: String) -> Int64 {
    let oprOrder = ["+", "-", "*"].permutation(r: 3)
    var result = [Int]()

    let num = expression.components(separatedBy: ["+", "-", "*"]).compactMap { Int($0) }
    let operators = expression.filter { !$0.isNumber }.map { String($0) }

    for i in 0..<oprOrder.count {
        var temp = num
        var ops = operators
        for j in 0..<oprOrder[i].count {
            while let index = ops.firstIndex(of: oprOrder[i][j]) {
                switch oprOrder[i][j] {
                case "+":
